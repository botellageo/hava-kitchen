/**
 * Tests des Security Rules pour `restaurants/{rid}/productTemplates/{pid}`.
 *
 * Couvre :
...
const PROJECT_ID = 'hava-kitchen-test-templates';
...
async function seedTemplate(rid: string, pid: string): Promise<void> {
  await testEnv.withSecurityRulesDisabled(async (ctx) => {
    await setDoc(doc(ctx.firestore(), `restaurants/${rid}/productTemplates/${pid}`), {
...
describe('firestore.rules — productTemplates/{pid}', () => {
  it('owner peut create + update + delete', async () => {
    ...
    await assertSucceeds(
      updateDoc(doc(ctx.firestore(), 'restaurants/r1/productTemplates/p1'), { dlcDays: 5 }),
    );
    await assertSucceeds(deleteDoc(doc(ctx.firestore(), 'restaurants/r1/productTemplates/p1')));
  });
  ...
  it('refuse lecture par un user sans relation au resto', async () => {
    ...
    await assertFails(getDoc(doc(ctx.firestore(), 'restaurants/r1/productTemplates/p1')));
  });
